...
void init(void)
{
    ...
    BYTE* image = LoadBitmapFile(&originalHeader, &imgSize, "pave.bmp"); //비트맵파일을 읽어 화소정보를 저장
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 500,
        483, 0, GL_RGBA, GL_UNSIGNED_BYTE,
        image);
}
...
int main(void)
{
    ...
    if (!glfwInit())
        exit(EXIT_FAILURE);
    window = glfwCreateWindow(680, 580, "Simple example", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        exit(EXIT_FAILURE);
